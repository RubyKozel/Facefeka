...
$(document).ready(function () {
  var base_url = _websiteUtils_properties_json__WEBPACK_IMPORTED_MODULE_2__.base_url;
  var routes = _websiteUtils_properties_json__WEBPACK_IMPORTED_MODULE_2__.routes;
  var error_cred = $('#error-cred');
  error_cred.hide();
  var form = $('#login-form');
  form.on('submit', function (e) {
    ...
    var rememberMe = form.find('input[id="remember-me"]').is(":checked");
    $.ajax({
      type: "POST",
      url: "".concat(base_url).concat(routes.sign_in),
      data: JSON.stringify({
        email: email,
        password: password
      }),
  ...
  if (localStorage.getItem('remember') === 'true') {
    $.ajax({
      type: "POST",
      url: "".concat(base_url).concat(routes.sign_in_auth),
      data: {},
      headers: {
  ...
  if (localStorage.getItem('remember') === 'false') {
    $.ajax({
      type: "DELETE",
      url: "".concat(base_url).concat(routes.remove_my_token),
      data: {},
      headers: {
        'x-auth': localStorage.getItem('x-auth')
      },
